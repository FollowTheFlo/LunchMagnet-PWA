...
import { ExtendedOrder, Order } from "./../../models/order.model";
import {
  Subject,
  Subscription,
  Observable,
...
export class StaffOrdersPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.ordersSub = this.staffService
      .fetchAllOrders()
      .pipe(switchMap((data) => this.staffService.orders$))
      .subscribe((orders) => {
  ...
  ionViewDidEnter() {
    ...
    this.intervalSub = interval(10000).subscribe((num) => {
  ...
  doRefresh(event) {
    ...
    this.staffService.fetchOrders_afterReset("").subscribe((orders) => {
  ...
  calculateDelay(nowDate: Date, orders: ExtendedOrder[]) {
    ...
    orders.forEach((order) => {
      const delayInMseconds =
        nowDate.getTime() - new Date(order.createdAt).getTime();
