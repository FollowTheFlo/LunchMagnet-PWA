...
export class TabsPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.authSub = this.authService.fetchUser('florent.letendre@gmail.com').pipe(
        switchMap(user => {
         // this.user = user;
        ...
        .subscribe( user => {
          ...
          if (this.user.role === 'USER' || this.user.role === 'ADMIN') {
            this.userOrdersSub =  this.orderService.fetchOrders(this.user._id).pipe(
          ...
          if (this.user.role === 'STAFF' || this.user.role === 'ADMIN') {
            ...
            this.staffOrdersSub = this.staffService.fetchAllOrders().pipe(
          ...
          if (this.user.role === 'DRIVER' || this.user.role === 'ADMIN') {
            ...
            let currentDriver;
            this.driverInitOrderSub = this.driverService.fetchDriver(this.user._id).pipe(
              switchMap (driver => {
            ...
            this.driverService.fetchDriver(this.user._id).pipe(
              switchMap (driver => {
                currentDriver = driver;
              ...
              this.driverService.fetchDriverOrders_afterReset(currentDriver._id)
              .subscribe();
