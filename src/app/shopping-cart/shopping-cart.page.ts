...
export class ShoppingCartPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.restaurantService.getTipsList().subscribe((tipsList) => {
    ...
    this.restaurantService.getTaxList().subscribe((taxList) => {
  ...
  getGrandTotal_UpdateTipsTaxes() {
    ...
    if (this.selectedTips) {
      this.selectedTips.amount =
        Math.round(this.subTotal * this.selectedTips.intValue) / 100;
      console.log("getGrandTotal2", this.selectedTips.intValue);
      price =
        Math.round(this.subTotal * (100 + this.selectedTips.intValue)) / 100;
    }
    // add taxes price
    this.taxList.forEach((tax) => {
      price += tax.amount;
  ...
  onAddTips(tip: ConfigItem) {
    ...
    this.selectedTips = tip;
    this.selectedTips.floatValue =
      Math.round(this.subTotal * tip.intValue) / 100;
    tip.selected = true;
    this.grandTotal = this.getGrandTotal_UpdateTipsTaxes();
  }
  ...
  onCheckoutStep() {
    ...
    this.headerTitle =
      this.checkoutStep === true ? "Checkout" : "Shopping Cart";
  ...
  onCheckout() {
    ...
    order.selectedMenuItems = JSON.parse(
      JSON.stringify(this.selectedMenuItems)
    );
    ...
    this.loadingCtrl
      .create({ keyboardClose: true, message: "Creating the order..." })
      .then((loadingEl) => {
        loadingEl.present();
