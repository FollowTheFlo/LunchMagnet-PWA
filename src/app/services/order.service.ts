...
export class OrderService {
  ...
  askDriverToTakeOrder(driverId: string, orderId: string) {
    ...
    return this.graphqlService
      .askDriverToTakeOrder(driverId, orderId)
      .pipe(map((response) => response.data.askDriverToTakeOrder));
  }
  ...
  assignDriverToOrder(driverId: string, orderId: string) {
    ...
    return this.graphqlService
      ...
      .pipe(map((response) => response.data.assignDriverToOrder));
  }
  ...
  completeOrderStep(orderId: string, index: number) {
    return this.graphqlService.completeOrderStep(orderId, index).pipe(
      ...
      tap((updatedOrder) => {
        const oIndex = this.orders.findIndex(
          (order) => order._id === updatedOrder._id
        );
        this.orders[oIndex] = Utils.deepCopyOrder(updatedOrder);
        this._orders.next(Utils.deepCopyOrdersList(this.orders));
      })
    );
  }
  ...
  cancelOrderStep(orderId: string, index: number) {
    return this.graphqlService.cancelOrderStep(orderId, index).pipe(
      ...
      tap((cancelOrder) => {
        const oIndex = this.orders.findIndex(
          (order) => order._id === cancelOrder._id
        );
