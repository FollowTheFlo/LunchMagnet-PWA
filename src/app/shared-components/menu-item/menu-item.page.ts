...
export class MenuItemPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.action === "add") {
      this.menuService.getOneMenuItem(this.menuItemId).subscribe(
        (item) => {
          this.menuItem = item;
      ...
      this.menuService.getOneSelectedItem(this.itemIndex).subscribe(
        (item) => {
          this.menuItem = Utils.deepCopyMenuItem(item);
          this.isLoading = false;
  ...
  updateOptionSelectionCount(option: Option) {
    return option.toppings.filter((toppingEl) => toppingEl.selected === true)
      .length;
  }
  ...
  async onBoxChanged(checkbox: any, topping: Topping, option: Option) {
    ...
    console.log(
      "filter selected el",
      option.toppings.filter((toppingEl) => toppingEl.selected === true).length
    );
    ...
    if (checkbox.checked) {
      ...
      const selectedCount = option.toppings.filter(
        (toppingEl) => toppingEl.selected === true
      ).length;

      // check if the exactNumber expected is smaller that the actual slected toppings
      if (
        option.exactNumber !== undefined &&
        option.exactNumber !== 0 &&
        option.exactNumber <= selectedCount
      ) {
        await this.presentToast(
          "Not possible: " + option.exactNumber + " selections required"
        );
      ...
      if (
        option.max !== null &&
        option.max !== 0 &&
        option.max <= selectedCount
      ) {
        await this.presentToast(
          "Not possible: " + option.max + " selections maximum"
        );
        // uncheck the box in DOM as choice not possible
        checkbox.checked = false;
        return;
  ...
  async editSelectedItem() {
    ...
    if (this.menuItem.quantity > 0) {
      ...
      this.menuService.editSelectedMenuItem(this.itemIndex, {
        ...this.menuItem,
      });
