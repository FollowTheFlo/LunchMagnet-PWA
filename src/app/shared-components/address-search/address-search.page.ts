...
export class AddressSearchPage implements OnInit, OnDestroy {
  ...
  onDismiss(value: string) {
    ...
    if (value === "save") {
      this.selectedAddress =
        this.selectedAddress === null ? "" : this.selectedAddress;
  ...
  drawRoad(lat: number, lng: number) {
    ...
    this.control = L.Routing.control({
      ...
      waypoints: [L.latLng(lat, lng), L.latLng(45.5832091, -73.5939564)],
    }).addTo(this.map);
  ...
  onSelectAddress(address: string) {
    ...
    if (this.newMarker !== undefined) {
      ...
      const newLatLng = new L.LatLng(
        this.customerFeature.geometry.coordinates[1],
        this.customerFeature.geometry.coordinates[0]
      );
      this.newMarker.setLatLng(newLatLng);
      //this.map = this.map.removeLayer(this.newMarker);
    } else {
